...
CREATE TABLE partnership_inquiries (
    ...
    user_id BIGINT,  -- 회원인 경우 (nullable)
...
COMMENT ON COLUMN partnership_inquiries.id IS '문의 ID';
COMMENT ON COLUMN partnership_inquiries.uuid IS '문의 UUID';
COMMENT ON COLUMN partnership_inquiries.user_id IS '회원 ID (비회원인 경우 NULL)';
...
COMMENT ON COLUMN partnership_inquiries.email IS '이메일';
COMMENT ON COLUMN partnership_inquiries.phone IS '연락처';
COMMENT ON COLUMN partnership_inquiries.content IS '문의 내용';
COMMENT ON COLUMN partnership_inquiries.status IS '처리 상태 (PENDING, IN_PROGRESS, COMPLETED, CANCELLED)';
COMMENT ON COLUMN partnership_inquiries.created_at IS '작성일시';
COMMENT ON COLUMN partnership_inquiries.updated_at IS '수정일시';
COMMENT ON COLUMN partnership_inquiries.is_deleted IS '삭제 여부';
COMMENT ON COLUMN partnership_inquiries.deleted_at IS '삭제일시';
COMMENT ON COLUMN partnership_inquiries.metadata IS '추가 데이터 (JSON)';